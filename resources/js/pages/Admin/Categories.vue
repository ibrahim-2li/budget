...
const colorOptions = [
    'text-green-500', 'text-secondary-500', 'text-blue-500', 'text-sky-500',
    'text-primary-500', 'text-purple-500', 'text-pink-500', 'text-rose-500',
...
function remove(category) {
    if (!confirm(t('Delete ":name"? This cannot be undone.', { name: t(category.name) }))) return
...
<template>
    ...
    <AdminLayout :title="t('Categories')" :subtitle="t(`Shared across every user's budget.`)">
        <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
            ...
            <section
                ...
                <header class="border-b border-gray-100 px-4 py-3.5 sm:px-5 dark:border-gray-800">
                    <h2 class="text-base font-semibold">{{ isEditing ? t('Edit category') : t('New category') }}</h2>
                </header>
                ...
                <form @submit.prevent="submit" class="space-y-4 px-4 py-4 sm:px-5">
                    ...
                    <div class="flex items-center gap-3 rounded-xl bg-gray-50 px-3 py-2.5 dark:bg-gray-800/60">
                        ...
                        <span class="truncate text-sm font-medium">{{ form.name || t('Category name') }}</span>
                    </div>

                    <div>
                        <label class="mb-1 block text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('Name') }}</label>
                        <input v-model="form.name" type="text" :placeholder="t('e.g. Groceries')"
                            class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm transition focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 focus:outline-none dark:border-gray-700 dark:bg-gray-800 dark:text-white"
                            :class="{ 'border-rose-400': form.errors.name }" />
                        <p v-if="form.errors.name" class="mt-1 text-xs text-rose-500">{{ form.errors.name }}</p>
                    </div>

                    <div>
                        <label class="mb-1 block text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('Type') }}</label>
                        <div class="grid grid-cols-2 gap-2">
                            <button v-for="type in ['income', 'expense']" :key="type" type="button" @click="form.type = type"
                                :class="[
                                    ...
                                    form.type === type
                                        ? type === 'income'
                                            ? 'border-secondary-500 bg-secondary-50 text-secondary-700 dark:bg-secondary-500/15 dark:text-secondary-300'
                    ...
                    <div>
                        <label class="mb-1 block text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('Icon') }}</label>
                        <div class="grid max-h-36 grid-cols-6 gap-1.5 overflow-y-auto rounded-lg border border-gray-200 p-2 dark:border-gray-700">
                            <button v-for="icon in iconOptions" :key="icon" type="button" @click="form.icon = icon"
                                :class="[
                                    ...
                                    form.icon === icon ? 'bg-primary-600 text-white' : 'hover:bg-gray-100 dark:hover:bg-gray-800',
                                ]">
                    ...
                    <div>
                        <label class="mb-1 block text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('Color') }}</label>
                    ...
                    <div class="flex gap-2">
                        <button type="submit" :disabled="form.processing"
                            class="flex-1 rounded-lg bg-primary-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-primary-500 active:scale-95 disabled:opacity-60">
            ...
            <div class="space-y-4 lg:col-span-2 lg:order-1">
                <section v-for="group in [
                    { label: 'Income', items: incomeCategories, dot: 'bg-secondary-500' },
                    ...
                    <ul class="divide-y divide-gray-100 dark:divide-gray-800">
                        <li v-for="category in group.items" :key="category.id"
                            ...
                            :class="{ 'bg-primary-50/60 dark:bg-primary-500/10': editingId === category.id }">
                            <span
                            ...
                            <div class="min-w-0 flex-1">
                                <p class="truncate text-sm font-medium">{{ t(category.name) }}</p>
                                <p class="text-xs text-gray-400 dark:text-gray-500">
                                    {{ t(category.usage_count === 1 ? ':count entry' : ':count entries', { count: category.usage_count }) }}
                                </p>
                            </div>
                            <button type="button" @click="startEdit(category)" :aria-label="t('Edit category')"
                                class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-gray-400 transition hover:bg-primary-50 hover:text-primary-600 dark:hover:bg-primary-500/15">
                                <i class="fa-solid fa-pen text-xs"></i>
                            </button>
                            <button type="button" @click="remove(category)" :disabled="category.usage_count > 0"
                                :title="category.usage_count > 0 ? t('In use by existing entries') : t('Delete')"
                        ...
                        <li v-if="group.items.length === 0" class="px-5 py-8 text-center text-sm text-gray-400">
                            {{ t(`No ${group.label.toLowerCase()} categories yet.`) }}
                        </li>
                    </ul>
                </section>
            </div>
